"""
E2E tests for TestRun endpoints.

These tests verify that TestRun endpoints work correctly by making
...
@pytest.mark.e2e
def test_create_testrun_endpoint():
    """Test POST /api/testrun endpoint and verify record persists in database."""
    token = get_auth_token()
    headers = {"Authorization": f"Bearer {token}"}
    data = {
        "name": "e2e-test-testrun",
        "description": "E2E test testrun document",
    }

    response = requests.post(f"{BASE_URL}/api/testrun", headers=headers, json=data)
    assert response.status_code == 201, _err(response, 201)

    response_data = response.json()
    assert "_id" in response_data, "Response missing '_id' key"
    assert response_data["name"] == "e2e-test-testrun"
    assert "created" in response_data
    assert "saved" in response_data


@pytest.mark.e2e
def test_get_testruns_endpoint():
    """Test GET /api/testrun endpoint."""
    token = get_auth_token()
    headers = {"Authorization": f"Bearer {token}"}
...
def test_get_testruns_with_name_filter():
    ...
    token = get_auth_token()
    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get(f"{BASE_URL}/api/testrun?name=e2e", headers=headers)
    assert response.status_code == 200, _err(response, 200)
...
@pytest.mark.e2e
def test_testrun_endpoints_require_auth():
