"""
E2E tests for TestData endpoints.

These tests verify that TestData endpoints work correctly by making
...
API runs on port 9096 (same for dev and api).
"""
import pytest
import requests
...
@pytest.mark.e2e
def test_create_testdata_endpoint():
    """Test POST /api/testdata endpoint and verify record persists in database."""
    token = get_auth_token()
    headers = {"Authorization": f"Bearer {token}"}
    data = {
        "name": "e2e-test-testdata",
        "description": "E2E test testdata document",
    }

    response = requests.post(f"{BASE_URL}/api/testdata", headers=headers, json=data)
    assert response.status_code == 201, _err(response, 201)

    response_data = response.json()
    assert "_id" in response_data, "Response missing '_id' key"
    assert response_data["name"] == "e2e-test-testdata"
    assert "created" in response_data
    assert "saved" in response_data


@pytest.mark.e2e
def test_get_testdatas_endpoint():
    """Test GET /api/testdata endpoint."""
    token = get_auth_token()
    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get(f"{BASE_URL}/api/testdata", headers=headers)
...
def test_get_testdatas_with_name_filter():
    ...
    token = get_auth_token()
    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get(f"{BASE_URL}/api/testdata?name=e2e", headers=headers)
    assert response.status_code == 200, _err(response, 200)
...
@pytest.mark.e2e
def test_testdata_endpoints_require_auth():
    """Test that testdata endpoints require authentication."""
    response = requests.get(f"{BASE_URL}/api/testdata")
