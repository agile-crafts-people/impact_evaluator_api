...
class TestServerInitialization(unittest.TestCase):
    ...
    def test_config_singleton_initialized(self, mock_get_config, mock_get_mongo, mock_signal):
        ...
        mock_config.EVALUATOR_API_PORT = 8184
...
class TestAppConfiguration(unittest.TestCase):
    ...
    def test_testrun_routes_registered(self):
        """Test that /api/testrun routes are registered."""
        response = self.client.get('/api/testrun')
        # Should not get 404 (route exists), but may get 401 (auth required)
        self.assertIn(response.status_code, [200, 401, 500])


    def test_testdata_routes_registered(self):
        """Test that /api/testdata routes are registered."""
    ...
    def test_grade_routes_registered(self):
        """Test that /api/grade routes are registered."""
    ...
    def test_all_blueprints_registered(self):
        ...
        self.assertIn('testrun_routes', blueprint_names)

        self.assertIn('testdata_routes', blueprint_names)


        self.assertIn('grade_routes', blueprint_names)
    ...
    def test_url_map_contains_expected_routes(self):
        ...
        self.assertTrue(any('/api/testrun' in rule for rule in rules))

        self.assertTrue(any('/api/testdata' in rule for rule in rules))


        self.assertTrue(any('/api/grade' in rule for rule in rules))
...
class TestServerExecution(unittest.TestCase):
    ...
    def test_main_execution_uses_config_port(self, mock_config, mock_run):
        """Test that __main__ execution uses EVALUATOR_API_PORT from config."""
        # Arrange
        mock_config.EVALUATOR_API_PORT = 9999
        ...
        if hasattr(server_module, '__name__'):
            ...
            api_port = mock_config.EVALUATOR_API_PORT
